    ...
    return (
        <div className={`bg-white/10 backdrop-blur-md p-6 shadow-2xl flex items-center h-full w-full overflow-hidden ${className}`}>
            <div className="flex items-center w-full gap-6">
                ...
                >
                    {logo ? (
                        <img src={logo} alt="Logo Masjid" className="h-32 w-32 object-cover rounded-full drop-shadow-lg" />
                    ) : (
                        <div className="h-32 w-32 bg-white/10 rounded-full flex items-center justify-center border-2 border-white/20">
                ...
                <div className="flex-1 flex flex-col justify-center items-start text-left">
                    <h1 className="text-5xl font-bold text-white tracking-wide drop-shadow-lg mb-1">{name}</h1>
                    <p className="text-2xl text-slate-300 font-light tracking-wider opacity-90">{address}</p>
                </div>
            </div>
        </div>
    );
